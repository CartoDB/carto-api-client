...
test('getAggregations - alias normalization with different casings', async () => {
  ...
  // Simulates different data warehouses that return aliases with varied casings
  const serverRow = {
    // BigQuery might return in UPPERCASE
    TOTAL_REVENUE: 50000,
    // PostgreSQL might maintain the original case
    Customer_Count: 125,
    // Snowflake might return everything in uppercase
    AVG_ORDER_VALUE: 400.5,
    // No alias, automatically generated by the backend
    max_rating: 5,
    // Some DWs might return in mixedCase
    minScore: 1.2,
  };
...
test('getAggregations - string format funciona para remote sources', async () => {
  ...
  // Server keys are normalized to lowercase
  expect(result).toEqual({
    rows: [
      {
        custom_metric: 42.7,
