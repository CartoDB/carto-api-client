...
import {applySorting} from '@carto/api-client';
...
describe('Sorting', () => {
  ...
  describe('should correctly understand sortOptions', () => {
    ...
    test('if sortBy is string', () => {
      expect(applySorting(features, {sortBy: 'column1'})).toEqual(
    ...
    test('if sortBy is array of arrays', () => {
      expect(
        applySorting(features, {sortBy: [['column1'], ['column2']]})
      ).toEqual(commonSortedFeatures);
    });
  });
